...
// --------------------- START ------------------------
      ...
      while ($rpenyakit = mysqli_fetch_array($sqlpenyakit)) {
        ...
        while ($rgejala = mysqli_fetch_array($sqlgejala)) {
          ...
          for ($i = 0; $i < count($_POST['kondisi']); $i++) {
            ...
            if ($rgejala['kode_gejala'] == $gejala) {
              $cf = ($rgejala['mb'] * $arbobot[$arkondisi[1]]) ;
              if (($cf >= 0) && ($cf * $cflama >= 0)) {
                ($cflama = $cflama + ($cf * (1 - $cflama)))/100;
              }
              if ($cf * $cflama < 0) {
                ($cflama = ($cflama + $cf) / (1 - Math . Min(Math . abs($cflama), Math . abs($cf))))/100;
              }
              if (($cf < 0) && ($cf * $cflama >= 0)) {
                ($cflama = $cflama + ($cf * (1 + $cflama)))/100;
              }
        ...
        if ($cflama > 0) {
          $arpenyakit += array($rpenyakit[kode_penyakit] => number_format($cflama *100 , 2));
        }
      }
...
// --------------------- END -------------------------
	...
	<h2 class='text text-primary'>Hasil Diagnosis &nbsp;&nbsp;<button id='print' onClick='window.print();' data-toggle='tooltip' data-placement='right' title='Klik tombol ini untuk mencetak hasil diagnosa'><i class='fa fa-print'></i> Cetak</button> </h2>
      ...
      foreach ($argejala as $key => $value) {
        ...
        echo '<tr><td>' . $ig . '</td>';
        echo '<td>K' . str_pad($r4[kode_gejala], 3, '0', STR_PAD_LEFT) . '</td>';
      ...
      echo "<div class='callout callout-default'>Jenis penyakit yang diderita adalah <b><h3 class='text text-success'>" . $nmpkt[1] . "</b> / " . round($vlpkt[1], 2) . " % <br></h3>";
      ...
      echo "
	 <h2 class='text text-primary'>Diagnosa Kerusakan HP</h2>  <hr>
	 <div class='alert alert-success alert-dismissible'>
                <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button>
                <h4><i class='icon fa fa-exclamation-triangle'></i>Perhatian !</h4>
                Silahkan memilih gejala sesuai dengan kondisi handphone anda, anda dapat memilih kepastian kondisi handphone dari pasti tidak sampai pasti ya, jika sudah tekan tombol proses (<i class='fa fa-search-plus'></i>)  di bawah untuk melihat hasil.
              </div>
      ...
      while ($r3 = mysqli_fetch_array($sql3)) {
        ...
        echo "<td class=opsi>K" . str_pad($r3[kode_gejala], 3, '0', STR_PAD_LEFT) . "</td>";
