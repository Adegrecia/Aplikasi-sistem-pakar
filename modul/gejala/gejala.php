...
switch($_GET[act]){
  ...
  $tampil=mysqli_query($conn,"SELECT * FROM gejala ORDER BY kode_gejala");
	echo "<form method=POST action='?module=gejala' name=text_form onsubmit='return Blank_TextField_Validator_Cari()'>
		  ...
		  <tr><td><input class='btn bg-olive margin' type=button name=tambah value='Tambah Kondisi' onclick=\"window.location.href='gejala/tambahgejala';\"><input type=text name='keyword' style='margin-left: 10px;' placeholder='Ketik dan tekan cari...' class='form-control' value='$_POST[keyword]' /> <input class='btn bg-olive margin' type=submit value='   Cari   ' name=Go></td> </tr>
          </table></form>";
	...
	if ($_POST[Go]){
			...
			if ($numrows > 0){
                ...
                Kondisi yang anda cari di temukan.
              </div>";
				$i = 1;
    ...
    echo "</tbody></table>";
                ...
                Maaf, Kondisi yang anda cari tidak ditemukan , silahkan inputkan dengan benar dan cari kembali.
              </div>";
			}
		}else{
  ...
  case "tambahgejala":
    echo "<form name=text_form method=POST action='$aksi?module=gejala&act=input' onsubmit='return Blank_TextField_Validator()'>
          <br><br><table class='table table-bordered'>
		  <tr><td width=120>Nama Kondisi</td><td><input type=text autocomplete='off' placeholder='Masukkan Kondisi baru...' class='form-control' name='nama_gejala' size=30></td></tr>
